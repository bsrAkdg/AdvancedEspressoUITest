...
import com.bsrakdg.blogpost.ui.viewmodel.*
import com.bsrakdg.blogpost.ui.viewmodel.state.MAIN_VIEW_STATE_BUNDLE_KEY
...
import com.bsrakdg.blogpost.util.ERROR_STACK_BUNDLE_KEY
        ...
        for (category in categories) {
            if (category.pk == menuItem.itemId) {
                ...
                if (category.category_name.equals(MENU_ITEM_NAME_GET_ALL_BLOGS)) {
                    viewModel.setStateEvent(MainStateEvent.GetAllBlogs())
                } else {
                    viewModel.setStateEvent(MainStateEvent.SearchBlogsByCategory(category.category_name))
                }
                return true
            }
        }
